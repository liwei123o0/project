...
def crawltest():
    ...
    for url in urls[:2]:
        ...
        try:
            ...
            print '未入库!'
            print "INSERT INTO CARHOME.news(IR_URLNAME,IR_URLTITLE,IR_URLTIME,IR_CONTENT,IR_SRCNAME,IR_GROUPNAME,IR_AUTHORS,IR_CATALOG2,IR_SITENAME) VALUES ('%s','%s','%s','%s','%s','房车微信','%s','房车微信','%s')"%(uri,title,date,content,gzh,gzh,gzh)
            print "Mysql Error %d: %s" % (e.args[0], e.args[1])
        driver.close()
        win1 = driver.window_handles
        driver.switch_to_window(win1[1])
